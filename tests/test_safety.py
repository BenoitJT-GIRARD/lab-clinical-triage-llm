...
from chsa_triage.evaluation.safety import check, summarize
...
def test_aucune_reponse_de_reference_n_est_signalee():
    ...
    signales = [
        ...
        if not check(
            f"{presentation.motif_fr} {presentation.signes_fr}",
            build_target_response(
                presentation.level,
                str(presentation.justification),
                str(presentation.recommandation),
            ),
            presentation.level,
        ).sans_defaut
    ]
...
# --- La recommandation doit porter l'immédiateté que le niveau exige ---

DESCRIPTION = "Homme de 62 ans, douleur thoracique. Constantes : TA 148/92, FC 102, SpO2 96 %."


def _rapport(niveau: str, recommandation: str, justification: str = "douleur thoracique."):
    reponse = (
...
def test_un_delai_trop_long_sur_une_urgence_vitale_est_incoherent(recommandation):
    ...
    """
    assert _rapport("URGENCE_VITALE", recommandation).recommandation_incoherente
...
def test_une_recommandation_reellement_immediate_reste_conforme(recommandation):
    assert not _rapport("URGENCE_VITALE", recommandation).recommandation_incoherente
...
def test_les_reponses_de_reference_du_catalogue_restent_conformes():
    ...
    fautives = [
        ...
        if not check(
            ...
            build_target_response(
                presentation.level, presentation.justification, presentation.recommandation
            ),
            presentation.level,
        ).sans_defaut
    ]
