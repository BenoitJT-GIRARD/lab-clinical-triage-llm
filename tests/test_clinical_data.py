...
from chsa_triage.data.case_generator import build_case, generate_cases
...
from chsa_triage.data.corpus_cases import build_corpus_case, extract_vignette
...
from chsa_triage.data.dpo_builder import build_preference_pairs
...
def test_chaque_presentation_est_complete():
    for presentation in PRESENTATIONS:
        ...
        assert presentation.motif_fr and presentation.motif_en
        assert len(presentation.signes_fr) >= 2 and len(presentation.signes_en) >= 2
        assert len(presentation.antecedents_fr) >= 2 and len(presentation.antecedents_en) >= 2
        assert presentation.delai in ("minutes", "heures", "jours", "semaines")
        assert len(presentation.justification) > 40
        assert len(presentation.recommandation) > 40
...
def test_une_presentation_qui_nomme_une_constante_l_impose(presentation):
    ...
    """
    imposees = dict(constantes_imposees(presentation.id))
...
def test_le_schema_publie_decrit_exactement_les_colonnes_livrees():
    ...
    """
    from chsa_triage.data.clinical_eval_set import eval_cases
    from chsa_triage.data.dataset_io import dpo_record, eval_record
