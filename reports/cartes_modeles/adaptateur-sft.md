...
# Agent de triage médical CHSA — adaptateur LoRA (fine-tuning supervisé)

Adaptateur LoRA issu du **fine-tuning supervisé** de `Qwen3-1.7B-Base` sur le
corpus de triage du CHSA. Il ne contient pas le modèle : il s'applique sur le
modèle de base.

Pour un usage direct, préférer le
[modèle final fusionné](https://huggingface.co/BenoitJT-GIRARD/qwen3-1.7b-chsa-triage),
...
modele = PeftModel.from_pretrained(base, "BenoitJT-GIRARD/qwen3-1.7b-chsa-triage-sft")
tokenizer = AutoTokenizer.from_pretrained("BenoitJT-GIRARD/qwen3-1.7b-chsa-triage-sft")
```
