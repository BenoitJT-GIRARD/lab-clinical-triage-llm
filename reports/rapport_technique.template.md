...
{{ENTREES_LUES}} entrées. UltraMedical-Preference, le cinquième, est réservé à
...
({{BUDGET_DESCRIPTION}} jetons). La troisième est une décision de sécurité : un
cas n'est conservé que si la règle y identifie explicitement un signe. Elle
écarte à elle seule {{SANS_SIGNE}} descriptions de patients authentiques, et rend
...
**Un catalogue de présentations cliniques.** {{NB_PRESENTATIONS}} présentations
...
| Jeu | Volume | Contenu |
|---|---|---|
| Entraînement supervisé | {{SFT_TRAIN}} | paires invite / réponse de triage |
| Validation | {{SFT_VALIDATION}} | suivi de la convergence |
| Test interne | {{SFT_TEST}} | même distribution que l'entraînement |
| Préférences (DPO) | {{DPO_TOTAL}} | triplets invite / préférée / rejetée |
| Évaluation clinique | {{EVAL_TOTAL}} | cas écrits à la main, jamais vus |

![Composition du dataset : équilibre des niveaux, et origine des exemples.](figures/01_composition_dataset.png)

Le corpus est **équilibré** : {{NIVEAU_PAR_CLASSE}} exemples par
niveau, {{LANGUE_FR}} en français et {{LANGUE_EN}} en anglais. {{PART_CONSTANTES}} des
...
**Origine des exemples.** {{COMPOSITION_ORIGINE}}

**Diversité des réponses attendues.** {{REPONSES_DISTINCTES}}

**Part du niveau lisible sur les métadonnées.** {{FUITE_METADONNEES}}
...
Mesure sur le dataset livré : {{FUITE}}.
...
{{ARRETS_NETS_REGLAGE}}. La pièce qui établit le diagnostic est la comparaison des
...
la sortie — ni le budget de génération — {{MEDIANE_REPONSE}} jetons en médiane pour un plafond de
...
{{JETONS_AVEC_TETE}} jetons au lieu de {{JETONS_SANS_TETE}} — **{{FACTEUR_JETONS}} fois
...
{{PARAMS_ENTRAINABLES}} paramètres sur {{PARAMS_TOTAL}}, soit
{{PART_ENTRAINABLE}} du modèle, ce qui fait tenir l'entraînement sur une carte de
16 Go.
...
| Rang LoRA · alpha · dropout | {{LORA_R}} · {{LORA_ALPHA}} · {{LORA_DROPOUT}} |
| Modules adaptés | projections d'attention et de perceptron, **et tête de sortie** |
| Taux d'apprentissage | {{SFT_LR}} |
| Époques · lot effectif | {{SFT_EPOCHS}} · {{SFT_LOT_EFFECTIF}} |
| Longueur de séquence | {{MAX_LENGTH}} jetons |
| Précision · gradient checkpointing | bfloat16 · activé (variante Unsloth) |
| Graine | {{SEED}} |

Perte d'entraînement finale {{SFT_TRAIN_LOSS}}, perte de validation
{{SFT_EVAL_LOSS}}. Cet écart quasi nul ne prouve pas l'absence de
...
configuration retenue est **{{VARIANTE_RETENUE}}**, la mieux placée sur cette
...
vient d'acquérir. Les {{DPO_TOTAL}} paires livrées sont donc construites à partir
...
1. **même format, même longueur.** Chaque défaut est décliné en plusieurs
   ...
   {{DPO_EQUILIBRE}} des paires — aucun signal de longueur exploitable ;
...
| Beta | {{DPO_BETA}} |
| Poids de la perte supervisée | {{DPO_RPO_ALPHA}} |
| Taux d'apprentissage | {{DPO_LR}} |
| Époques · lot effectif | {{DPO_EPOCHS}} · {{DPO_LOT_EFFECTIF}} |
...
correctement **{{DPO_PRECISION}}** des paires ; les courbes d'entraînement seules
...
{{EVAL_TOTAL}} cas écrits à la main, jamais vus à l'entraînement, dont les
étiquettes proviennent d'un raisonnement clinique et non de la règle à laquelle
le modèle est comparé. {{PART_PIEGES}} d'entre eux sont des **présentations
atypiques** :
...
Deux conséquences. Sur {{EVAL_TOTAL}} cas, un écart de trois points n'est pas un
...
**lignes**, pas des présentations : les {{PRESENTATIONS_TEST}} présentations
...
n-grammes atteint {{SEPARABILITE_ALEATOIRE}} sur les vignettes en découpage
aléatoire et {{SEPARABILITE_GROUPEE}} en découpage par présentation d'origine.
Les {{SEPARABILITE_ECART}} points qui séparent ces deux chiffres sont la part de
...
**Un contrôle de sécurité doit lui-même être contrôlé.** Les {{NB_PRESENTATIONS}}
...
Accord entre les deux jeux de prédictions : **{{ACCORD_LIVRAISONS}}**. Les écarts
...
|---|---|---|
| Réglage des hyperparamètres | {{REGLAGE_DUREE}} | {{REGLAGE_MEMOIRE}} |
| Fine-tuning supervisé | {{SFT_DUREE}} | {{SFT_MEMOIRE}} |
| Alignement par préférences | {{DPO_DUREE}} | {{DPO_MEMOIRE}} |
...
**Le jeu d'évaluation est petit.** {{EVAL_TOTAL}} cas donnent des intervalles de
...
| # | Critère | Seuil | État |
|---|---|---|---|
| 1 | Catalogue clinique relu et validé par deux urgentistes | 100 % des présentations | {{GONOGO_1}} |
| 2 | Jeu d'évaluation annoté indépendamment par deux soignants | accord inter-annotateurs ≥ 0,8 | {{GONOGO_2}} |
| 3 | Sous-triage sur le jeu clinique | borne haute de l'IC 95 % ≤ 5 % | {{GONOGO_3}} |
| 4 | Respect du format | borne basse de l'IC 95 % ≥ 99 % | {{GONOGO_4}} |
| 5 | Latence 95ᵉ centile à quatre requêtes simultanées | ≤ 2 000 ms | {{GONOGO_5}} |
| 6 | Journal d'audit persistant et anonymisé | vérifié en conditions réelles | {{GONOGO_6}} |
| 7 | Endpoint authentifié, limité en débit, derrière HTTPS | vérifié | {{GONOGO_7}} |
| 8 | Procédure de retour arrière éprouvée | testée sur l'environnement pilote | {{GONOGO_8}} |
| 9 | Analyse d'impact relative à la protection des données | validée par le DPO du CHSA | {{GONOGO_9}} |
| 10 | Qualification réglementaire du dispositif | statut tranché avec le service juridique | {{GONOGO_10}} |
...
Graine fixée à {{SEED}}, hyperparamètres versionnés, exécutions d'entraînement
...
Les chiffres de ce rapport viennent de la révision {{REVISION}} du dépôt :
`Qwen/Qwen3-1.7B-Base` spécialisé en `{{MODELE_LIVRE}}`, entraîné et évalué sur
{{MATERIEL}}.

Bibliothèques relevées à l'exécution : {{BIBLIOTHEQUES}}.
