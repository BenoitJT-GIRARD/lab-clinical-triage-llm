...
def test_aucun_taux_n_est_publie_sous_six_observations():
    ...
    """
    assert intervalle_de_proportion(3, 3) is None
    assert intervalle_de_proportion(4, 4) is None
    assert intervalle_de_proportion(5, 5) is None
    assert intervalle_de_proportion(6, 6) is not None


def test_la_politique_choisit_l_exact_puis_wilson():
    assert intervalle_de_proportion(19, 20) == clopper_pearson(19, 20)
    assert intervalle_de_proportion(51, 60) == wilson_interval(51, 60)
