...
from chsa_triage.config import DATA, MODEL, PATHS, SEED, SERVING, TRAINING, TRIAGE
from chsa_triage.utils import chemin_pour_journal, get_logger, set_seed
...
def test_les_chemins_du_projet_sont_journalises_relativement_a_la_racine():
    assert chemin_pour_journal(PATHS.sft_merged) == "models/qwen3-1.7b-triage-sft-merged"
    assert chemin_pour_journal(str(PATHS.data_processed)) == "data/processed"


def test_ce_qui_n_est_pas_un_chemin_du_projet_est_journalise_tel_quel():
    assert chemin_pour_journal(MODEL.base_model) == MODEL.base_model
    ailleurs = Path(PATHS.root.anchor) / "modeles" / "qwen3"
    assert chemin_pour_journal(ailleurs) == str(ailleurs)
