...
```bash
cp -r models/qwen3-1.7b-triage-dpo-merged ~/.local/share/chsa-triage/models/
TRIAGE_MODELS_DIR=~/.local/share/chsa-triage/models \
  docker compose -f deploy/docker-compose.yml up --build
```
...
```bash
TRIAGE_RATE_LIMIT=6000 docker compose -f deploy/docker-compose.yml up -d
```
...
uv run uvicorn chsa_triage.serving.api:app --port 8080
```
...
uv run modal secret create chsa-triage \
...
```bash
uv run modal secret create chsa-triage --force \
    TRIAGE_API_KEY="une-cle-choisie" \
    HF_TOKEN="hf_..." \
    TRIAGE_VLLM_URL="https://<compte>--chsa-triage-moteur.modal.run"
uv run modal deploy deploy/modal_app.py
```
...
`deploy/modal_app.py` s'exécute sur le poste et lit son environnement dès
...
```bash
uv run python scripts/08_publish_hf.py --what tout     # poids, dataset, cartes
...
```bash
vllm serve <compte>/qwen3-1.7b-chsa-triage --revision modele-v1.0.0 \
  --served-model-name qwen3-1.7b-chsa-triage --max-model-len 1024
```
...
GitHub : ils vivent dans le secret Modal `chsa-triage`, créé plus haut. GitHub
...
| `TRIAGE_VLLM_MODEL` | nom du modèle servi par vLLM | `qwen3-1.7b-chsa-triage` |
...
| `TRIAGE_MODEL_VERSION` | version inscrite au journal d'audit | `qwen3-1.7b-chsa-triage` |
...
deploy/docker-compose.yml` cherche son fichier d'environnement dans `deploy/`,
et la commande `modal` charge `deploy/modal_app.py` hors du processus qui lit
...
| `TRIAGE_MODEL_ID` | `modal deploy` | dépôt du modèle servi | `<HF_NAMESPACE>/qwen3-1.7b-chsa-triage` |
...
```bash
docker compose -f deploy/docker-compose.yml cp api:/app/logs/audit_triage.jsonl .
```
